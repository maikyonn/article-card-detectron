...
def register_dataset(cfg):
    for d in ["train", "val"]:
        json_file = os.path.join(cfg.DATA_DIR, "annotations", f"{d}_annotations.json")
        DatasetCatalog.register(f"article_card_{d}", lambda f=json_file: get_article_card_dicts(f))
        MetadataCatalog.get(f"article_card_{d}").set(thing_classes=["article_card"])
        MetadataCatalog.get(f"article_card_{d}").evaluator_type = "coco"  # Ensure evaluator type is set
